...
if value>=abs(n) and hunger>=abs(n):
    ch.mod_cc(CC, -abs(n))
    ch.mod_cc(EE, -abs(n))
    text=f''' -title "{name} eats what they can!" -f "{CC}|{ch.cc_str(CC)} (-{abs(n)})" -f "{EE}|{ch.cc_str(EE)} (-{abs(n)})" '''
else:
    ch.mod_cc(CC, -min(value, hunger))
    ch.mod_cc(EE, -min(value, hunger))
    text=f''' -title "{name} eats what they can!" -f "{CC}|{ch.cc_str(CC)} (-{min(value, hunger)})" -f "{EE}|{ch.cc_str(EE)} (-{min(value, hunger)})" '''
return text+refill
</drac2> -thumb {{image}}
