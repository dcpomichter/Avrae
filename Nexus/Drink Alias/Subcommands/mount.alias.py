...
ff="Thirst-Mount"
for x in mount:
    real=ch.cc_exists(f"""Thirst-{x.title()}""")
    if real:
        ff=f"""Thirst-{x.title()}"""
        break
...
if value>=abs(n) and thirst>=abs(n):
    ch.mod_cc(dd, -abs(n))
    ch.mod_cc(ff, -abs(n))
    text=f''' -title "{name} lets their mount drink what they can!" -f "{dd}|{ch.cc_str(dd)} (-{abs(n)})" -f "{ff}|{ch.cc_str(ff)} (-{abs(n)})" '''
else:
    ch.mod_cc(dd, -min(value, thirst))
    ch.mod_cc(ff, -min(value, thirst))
    text=f''' -title "{name} lets their mount drink what they can!" -f "{dd}|{ch.cc_str(dd)} (-{min(value, thirst)})" -f "{ff}|{ch.cc_str(ff)} (-{min(value, thirst)})" '''
return text+refill
</drac2> -thumb {{image}}
