...
coinpurse=''
if not i:
    ch.coinpurse.modify_coins(sp=-5)
    coinpurse=f""" -f "Coinpurse|{ch.coinpurse.compact_str()} (-0.5)" """
...
for hd in HD_types:
        if ch.cc_exists(hd):
            ch.set_cc(hd, (ch.get_cc_max(hd)//4))
HD_output='\n'.join(f"{x}: {ch.cc_str(x)}" for x in HD_types)
text=f''' -title "{name} lives a Squalid life." -f "{EE}| {ch.cc_str(EE)}" -f "{ff}| {ch.cc_str(ff)}" -f "{FF}| {ch.cc_str(FF)}" {output} -f "{name} new Max HP| {ch.hp}" -f "Starting Hit Dice| {HD_output}" {coinpurse}'''
return text
</drac2> -thumb {{image}}
