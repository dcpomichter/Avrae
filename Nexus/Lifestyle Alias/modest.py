...
if not i:
    if nexian:
        ch.coinpurse.modify_coins(gp=-4)
        coinpurse=f""" -f "Coinpurse|{ch.coinpurse.compact_str()} (-4)" """
    else:
        ch.coinpurse.modify_coins(gp=-8)
        coinpurse=f""" -f "Coinpurse|{ch.coinpurse.compact_str()} (-8)" """
ch.set_hp(ch.max_hp)
...
for hd in HD_types:
        if ch.cc_exists(hd):
            ch.set_cc(hd, (ch.get_cc_max(hd)*0.75))
HD_output='\n'.join(f"{x}: {ch.cc_str(x)}" for x in HD_types)
text=f''' -title "{name} lives a Modest life." -f "{EE}| {ch.cc_str(EE)}" -f "{ff}| {ch.cc_str(ff)}" -f "{FF}| {ch.cc_str(FF)}" {output} -f "{name} new Max HP| {ch.hp}" -f "Starting Hit Dice| {HD_output}" {coinpurse} '''
return text
</drac2> -thumb {{image}}
