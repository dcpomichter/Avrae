...
coinpurse=''
if not i:
    ch.coinpurse.modify_coins(gp=-30)
    coinpurse=f""" -f "Coinpurse|{ch.coinpurse.compact_str()} (-30)" """
ch.set_hp((ch.max_hp)*1.10)
...
text=f''' -title "{name} lives a Wealthy life." -f "{EE}| {ch.cc_str(EE)}" -f "{ff}| {ch.cc_str(ff)}" -f "{FF}| {ch.cc_str(FF)}" {output} -f "{name} new Max HP| {ch.hp}" -f "Starting Hit Dice| {HD_output}" {coinpurse} '''
return text
</drac2> -thumb {{image}}
