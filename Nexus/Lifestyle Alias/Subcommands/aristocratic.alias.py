...
coinpurse=''
if not i:
    ch.coinpurse.modify_coins(gp=-70)
    coinpurse=f""" -f "Coinpurse|{ch.coinpurse.compact_str()} (-70)" """
ch.set_hp((ch.max_hp)*1.20)
...
for hd in HD_types:
        if ch.cc_exists(hd):
            ch.set_cc(hd, (ch.get_cc_max(hd)))
HD_output='\n'.join(f"{x}: {ch.cc_str(x)}" for x in HD_types)
text=f''' -title "{name} lives an Aristocratic life." -f "{EE}| {ch.cc_str(EE)}" -f "{ff}| {ch.cc_str(ff)}" -f "{FF}| {ch.cc_str(FF)}" {output} -f "{name} new Max HP| {ch.hp}" -f "Starting Hit Dice| {HD_output}" {coinpurse} '''
return text
</drac2> -thumb {{image}}
