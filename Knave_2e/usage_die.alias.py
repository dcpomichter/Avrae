...
ch, n, args=character(), "\n", &ARGS&
cc=f"{'&1&'.title()} Die"
desc= f"Instead of tracking each individual item, take a die - this is your usage die. Roll it whenever you use that item: if you roll a 1 or 2, the die gets one size smaller. {n}If you're down to one piece of ammunition and you use it, that's it - it's all gone! Remove it from your inventory. {n}{n}Usage Die: d12 → d10 → d8 → d6 → d4 → 1"
ch.create_cc_nx(cc, 0, 7, None, 'square', 4, desc=desc, initial_value=4)
