...
for cls, lvl in ch.levels:
    ...
    for counter in counters:
        if sub:
            if counter.subclass in sub:
                if lvl>=counter.level:
                    maxValue=str("("+(counter.maxValue[lvl-counter.level] if typeof(counter.maxValue)=='SafeList' else counter.maxValue)+" * "+counter.get("multiplier",1)+")"+" + "+counter.get("modifier", 0))
                    ch.create_cc_nx(name=counter.name,minVal=0,maxVal=maxValue,reset=counter.reset,dispType=counter.display,reset_to=counter.get("resetto",maxValue),desc=counter.desc)
                    if ch.get_cc_max(counter.name)!=maxValue:
                        ch.edit_cc(counter.name,maxVal=maxValue)
...
ch=character()
n="\n"
classes=load_json(get_gvar("ce7fa7e8-8e91-411f-89b8-40f5e25aac8d"))|load_json(get_gvar("b4376b32-7fd9-4114-aca8-91ae228b3f19"))|load_json(get_gvar("2c942348-da36-4b3c-8585-e7d61e2cfabc"))|load_json(get_gvar("a483953f-8335-4d64-a0dd-09f2c9107928"))
currentSubclasses=load_json(get("subclass", {}))
currentAttackList=[x.name for x in ch.attacks]
out=[]
for cls, lvl in ch.levels:
    cls_info=classes.get(cls, {})
    sub=currentSubclasses.get(f"{cls}Level")
    ready=cls_info.get("None", [])
    if sub:
        sub_actions=cls_info.get(sub, [])
        for automation in sub_actions:
            if automation.action.name not in currentAttackList and automation.level<=lvl:
                out.append(dump_json(automation.action))
    for automation in ready:
        if automation.action.name not in currentAttackList and automation.level<=lvl:
            out.append(dump_json(automation.action))
base=load_json(get_gvar("0d536e07-164e-4583-9584-7fe12f47fcba"))
for action in base:
    if action.name not in currentAttackList:
        out.append(dump_json(action))
if not out:
  return f"""{ctx.prefix}techo 20 You already have all your actions """
return f'''{ctx.prefix}a import ''' + f'''\n{ctx.prefix}a import '''.join(out)
</drac2>
